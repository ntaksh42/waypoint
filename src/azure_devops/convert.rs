...
/// Howard Hinnant の `civil_from_days` アルゴリズム。`days_from_civil` の逆変換
/// (1970-01-01 を 0 とする通算日数からグレゴリオ暦の年月日を求める)。
...
/// unix 秒を WIQL の日時リテラル (`'YYYY-MM-DD HH:MM:SS'`、UTC) へ変換する。
/// `[System.ChangedDate] > '...'` のような差分クエリの下限に使う。
pub(crate) fn unix_to_wiql_datetime(unix_seconds: i64) -> String {
    let days = unix_seconds.div_euclid(86_400);
    let remainder = unix_seconds.rem_euclid(86_400);
...
mod tests {
    ...
    #[test]
    fn unix_to_wiql_datetime_matches_known_epoch_values() {
        assert_eq!(unix_to_wiql_datetime(0), "1970-01-01 00:00:00");
        assert_eq!(unix_to_wiql_datetime(1_705_314_600), "2024-01-15 10:30:00");
        // うるう年 (2024) の 2/29 をまたぐ日付も正しく変換できること
        assert_eq!(unix_to_wiql_datetime(1_709_251_200), "2024-03-01 00:00:00");
    }

    #[test]
    fn unix_to_wiql_datetime_round_trips_with_parse_rfc3339_unix() {
        for seconds in [0, 1_705_314_600, 1_709_251_200, 1_800_000_000] {
            let wiql = unix_to_wiql_datetime(seconds);
            let rfc3339 = format!("{}Z", wiql.replacen(' ', "T", 1));
            assert_eq!(parse_rfc3339_unix(&rfc3339), Some(seconds));
        }
    }
}
